...
//#define __SHOW_PIPE_TO_USB_LOG			
	  ...
	  unsigned int __ENGINE_LOWRANGE_SPREADS[TOTAL_CHIPS_INSTALLED][16];
	  unsigned int __ENGINE_HIGHRANGE_SPREADS[TOTAL_CHIPS_INSTALLED][16];
#endif

#if defined(__SHOW_DECOMMISSIONED_ENGINES_LOG)
	  char szDecommLog[4096];
#endif
...
#else	
	#define __ASIC_FREQUENCY_ACTUAL_INDEX   4 
#endif
...
// We define our u32 and Unsigned Long Long
...
//  unsigned int GLOBAL_SCATTERED_DIAG_TIME;
...
#define ASIC_SPI_RW_COMMAND						    0x8000

// Our Wathdog Reset command
//#define WATCHDOG_RESET  (*(u32*)0x0FFFFFFFF)= 0x0//AVR32_WDT.clr = 0x0FFFFFFFF

#define WATCHDOG_RESET NOP_Process()			//== will add code later
...
// FAN Controls
#define FAN_CONTROL_BYTE_VERY_SLOW			(0x02)
#define FAN_CONTROL_BYTE_SLOW				(0x04)
#define FAN_CONTROL_BYTE_MEDIUM				(0x08)
#define FAN_CONTROL_BYTE_FAST				(0x01)
#define FAN_CONTROL_BYTE_VERY_FAST			(0x0F)
...
/*
  unsigned int DEBUG_TraceTimer0;
  unsigned int DEBUG_TraceTimer1;
  unsigned int DEBUG_TraceTimer2;
  unsigned int DEBUG_TraceTimers[8];
  unsigned int DEBUG_TraceTimersIndex;

  unsigned int DEBUG_LastXLINKTransTook;*/

// Assembly NOP operation
#ifdef __OPERATING_FREQUENCY_64MHz__
	#define NOP_OPERATION asm  ("nop \n\t nop \n\t nop \n\t nop \n\t nop \n\t");
