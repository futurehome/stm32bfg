...
void __AVR32_LowLevelInitialize()
{
	int i_delay = 0;
	// ********* Enable OSC0
	AVR32_MCCTRL  |= 0x04; // OSC0EN = 1
	AVR32_CKSEL   = 0; // All Clocks equal main-clock
	AVR32_OSCCTRL0 = 0x00000107;
	
	// Enable watchdog to reset the system if PLL fails
	////JJ// AVR32_WDT.ctrl = (0x055000000) | (0xF00) | 0x01; // Timeout is set to 142ms
	////JJ// NOP_OPERATION;
	////JJ// AVR32_WDT.ctrl = (0x0AA000000) | (0xF00) | 0x01;
	////JJ// NOP_OPERATION;
	////JJ// AVR32_WDT.clr = 0x0FFFFFFFF;
	...
	while ((AVR32_POSCSR & (1 << 7)) == 0);
		...
		#else
			AVR32_PLL0 = 0x1F070205; // PLL0 at 64MHz
		#endif		 	   
	...
	#elif defined(__OPERATING_FREQUENCY_64MHz__)
		AVR32_GCCTRL = 0x00000006; // This is for 64MHz, with DIVEN=0
		// Second bit in this block is our PLL-SEL=1, OSCSEL=0, DIVEN=0, DIV=0
	#elif defined(__OPERATING_FREQUENCY_48MHz__)
	...
	#elif defined(__OPERATING_FREQUENCY_64MHz__)
		AVR32_MCCTRL |= 0x02; 	// MCSEL = 1 -- PLL0 Selected		
	#elif defined(__OPERATING_FREQUENCY_48MHz__)
		AVR32_MCCTRL |= 0b010; 	// MCSEL = 1 -- PLL0 Selected			
	#endif
...
{
	// Initialize A2D unit
	AVR32_A2D_MODE_REGISTER = MAKE_DWORD(0x08, // Setup/Hold Time = 1000 (binary)
										0x08, // Statup Time = 1000 (binar)
										0x09, // PRESCALE set to 9 (effect is (9+1) * 2 = 20)
										0x00); // No Trigger, resultion = 10 bits, no RESET or SLEEP
...
void __AVR32_USB_Initialize()
{
	...
	////JJ// AVR32_GPIO.port[0].gpers =  __AVR32_USB_AD0 | __AVR32_USB_AD1 | __AVR32_USB_AD2 | __AVR32_USB_AD3 |
	////JJ//							__AVR32_USB_AD4 | __AVR32_USB_AD5 | __AVR32_USB_AD6 | __AVR32_USB_AD7 |
	////JJ//							__AVR32_USB_WR  | __AVR32_USB_RD;
	////JJ// 
	////JJ// AVR32_GPIO.port[0].oders = 	__AVR32_USB_WR	| __AVR32_USB_RD;
	////JJ// AVR32_GPIO.port[0].ovrs =   __AVR32_USB_WR	| __AVR32_USB_RD;
	
	// Enable GPIOs on Port C
	////JJ// AVR32_GPIO.port[2].gpers =  __AVR32_USB_SIWUA |	__AVR32_USB_A0 | __AVR32_USB_CS;
	////JJ// AVR32_GPIO.port[2].oders =  __AVR32_USB_SIWUA |	__AVR32_USB_A0 | __AVR32_USB_CS;
	
	// Permanently select this chip
	////JJ// AVR32_GPIO.port[0].ovrc  =  __AVR32_USB_A0;
}
*/
/*
...
/*
u16 __AVR32_USB_WriteData(char* iData, unsigned int iCount)
{
	//unsigned int iRevCounter;
	//unsigned int vx;
	// Return if we're zero
	if (iCount == 0) return 0;
	
	// Write data...
	////JJ// AVR32_GPIO.port[0].oders =  __AVR32_USB_AD0 | __AVR32_USB_AD1 | __AVR32_USB_AD2 | __AVR32_USB_AD3 |
	////JJ// 							__AVR32_USB_AD4 | __AVR32_USB_AD5 | __AVR32_USB_AD6 | __AVR32_USB_AD7;
	////JJ// NOP_OPERATION;*/
	/*
	for (vx = 0; vx < iCount; vx++)
	{
		// Is there space available?
		if ((__AVR32_USB_GetInformation() & 0x02) == FALSE) 
		{
			...
			iRevCounter = 0x0010F0000;
			while ((iRevCounter-- > 0) && ((__AVR32_USB_GetInformation() & 0x02) == FALSE))
			{
				// Do nothing
			}
		...
		// Write data
		////JJ// AVR32_GPIO.port[0].ovr  = (AVR32_GPIO.port[0].ovr & 0x0FFFFFF00) | iData[vx];
		////JJ// NOP_OPERATION;
		////JJ// AVR32_GPIO.port[0].ovrc = __AVR32_USB_WR;
		////JJ// NOP_OPERATION;
		////JJ// AVR32_GPIO.port[0].ovrs = __AVR32_USB_WR;
		////JJ// NOP_OPERATION;
		
	}
	
	// Disable output drivers on bus lines
	////JJ// AVR32_GPIO.port[0].oderc =  __AVR32_USB_AD0 | __AVR32_USB_AD1 |	__AVR32_USB_AD2 | __AVR32_USB_AD3 |
	////JJ// 							__AVR32_USB_AD4 | __AVR32_USB_AD5 |	__AVR32_USB_AD6 | __AVR32_USB_AD7;
	////JJ// NOP_OPERATION;
	// Return
	return iCount;*/
/*
	return __ARM_UsbSendData((u8*)iData, iCount);
}*/

/*
u8  __AVR32_USB_GetInformation()
{
	// Save actual port state
	////JJ//   int iActualPortState = (AVR32_GPIO.port[0].oder & 0x0FF);
	//u8 iRetVal = 0;
	...
	// Set A0
	////JJ// AVR32_GPIO.port[2].ovrs =  __AVR32_USB_A0;
	////JJ// NOP_OPERATION;
	
	// Get the value
	////JJ// AVR32_GPIO.port[0].ovrc = __AVR32_USB_RD;
	////JJ// NOP_OPERATION;
	////JJ// iRetVal  = ((AVR32_GPIO.port[0].pvr) & 0x0FF);
	////JJ// NOP_OPERATION;
	////JJ// AVR32_GPIO.port[0].ovrs = __AVR32_USB_RD;
	////JJ// NOP_OPERATION;
	
	// Clear A0
	////JJ// AVR32_GPIO.port[2].ovrc =  __AVR32_USB_A0;
	////JJ// NOP_OPERATION;
	
	// Restore state
	////JJ// AVR32_GPIO.port[0].oder = (AVR32_GPIO.port[0].oder & 0x0FFFFFF00) | iActualPortState;
	////JJ// NOP_OPERATION;
...
char __AVR32_USB_GetData(char* iData, char iMaxCount)
{
	...
	////JJ//   unsigned int i_total_read = 0;
	for (vx = 0; vx < iMaxCount; vx++)
	{
		// Is there data?
...
void __AVR32_USB_FlushInputData()
{
	...
	// Write as many bytes as needed
	//while ((__AVR32_USB_GetInformation() & 0x01) == 0x01)
	//{
		// Get the value
		////JJ// AVR32_GPIO.port[0].ovrc = __AVR32_USB_RD;
		////JJ// NOP_OPERATION;
...
{
	// Set A0
	////JJ// AVR32_GPIO.port[2].ovrs =  __AVR32_USB_SIWUA;
	////JJ// NOP_OPERATION;
	////JJ// AVR32_GPIO.port[2].ovrc =  __AVR32_USB_SIWUA;
	////JJ// NOP_OPERATION;
}*/
...
void __AVR32_CPLD_SetAccess()
{
	...
	////JJ// AVR32_GPIO.port[0].ovrc = __AVR32_CPLD_CS; // Activate CS and leave it active...
}

void __AVR32_CPLD_Write (char iAdrs, char iData)
{
	// Disable CPLDs OE and Select the chip
	////JJ// AVR32_GPIO.port[1].ovrc = __AVR32_CPLD_OE;
	
	// Activate bus output first AND set the ADRS pin
	////JJ// AVR32_GPIO.port[1].oders = __AVR32_CPLD_BUS_ALL;
	
	// Set address
	////JJ// AVR32_GPIO.port[1].ovrs = __AVR32_CPLD_ADRS;
	////JJ// AVR32_GPIO.port[1].ovr  = (AVR32_GPIO.port[1].ovr & 0x0FFFFFF00) | iAdrs;
	
	// Strobe
	////JJ// AVR32_GPIO.port[0].ovrs = __AVR32_CPLD_STROBE;
	////JJ// AVR32_GPIO.port[0].ovrc = __AVR32_CPLD_STROBE;
	
	// Set Data and disable address
	////JJ// AVR32_GPIO.port[1].ovrc = __AVR32_CPLD_ADRS;
	////JJ// AVR32_GPIO.port[1].ovr  = (AVR32_GPIO.port[1].ovr & 0x0FFFFFF00) | iData;
	...
	////JJ// AVR32_GPIO.port[1].oderc = __AVR32_CPLD_BUS_ALL;
}
...
void __AVR32_CPLD_StartTX(char iTxControlValue)
{
	...
	// Set address
	////JJ// AVR32_GPIO.port[1].ovrs = __AVR32_CPLD_ADRS;
	////JJ// AVR32_GPIO.port[1].ovr  = (AVR32_GPIO.port[1].ovr & 0x0FFFFFF00) | 34; // 34 is CPLD_TX_ADDRESS_COTROL
	...
	////JJ// XLINK_activate_address_increase;

	// Write data
	////JJ//   unsigned int iInitialOvrValue = (AVR32_GPIO.port[1].ovr & 0x0FFFFFF00);
	////JJ// AVR32_GPIO.port[1].ovr  = (iInitialOvrValue) | iTxControlValue;
	////JJ// AVR32_GPIO.port[0].ovrs = __AVR32_CPLD_STROBE;
	////JJ// AVR32_GPIO.port[0].ovrc = __AVR32_CPLD_STROBE;
	////JJ// AVR32_GPIO.port[1].ovr  = (iInitialOvrValue) | 0b01;
	////JJ// AVR32_GPIO.port[0].ovrs = __AVR32_CPLD_STROBE;
	////JJ// AVR32_GPIO.port[0].ovrc = __AVR32_CPLD_STROBE;

	// Disable Address-Increase
	////JJ// XLINK_deactivate_address_increase;
...
void __AVR32_CPLD_BurstTxWrite(char* iData, char iAddress)
{
	...
	// Set address
	////JJ// AVR32_GPIO.port[1].ovrs = __AVR32_CPLD_ADRS;
	////JJ// AVR32_GPIO.port[1].ovr  = (AVR32_GPIO.port[1].ovr & 0x0FFFFFF00) | iAddress;
	...
	// Write data
	/*
	  unsigned int iInitialOvrValue = (AVR32_GPIO.port[1].ovr & 0x0FFFFFF00);
...
void __AVR32_CPLD_BurstRxRead(char* iData, char iAddress)
{
	...
	// Activate OE
	////JJ// AVR32_GPIO.port[1].ovrs = __AVR32_CPLD_OE;
	
	// Activate Address-increase
	////JJ// XLINK_activate_address_increase;
	
	// Get the result
	////JJ// iData[0] = (AVR32_GPIO.port[1].pvr & 0x000000FF);
	...
	// Get the result
	////JJ// iData[1] = (AVR32_GPIO.port[1].pvr & 0x000000FF);
	...
	// Get the result
	////JJ// iData[2] = (AVR32_GPIO.port[1].pvr & 0x000000FF);
	...
	// Get the result
	////JJ// iData[3] = (AVR32_GPIO.port[1].pvr & 0x000000FF);
...
unsigned int	__AVR32_CPLD_Read (char iAdrs)
{
	...
	// Get the result
	////JJ//   int iRetVal = (AVR32_GPIO.port[1].pvr & 0x000000FF);
...
void __AVR32_SC_Initialize()
{
	...
	*/
	...
	AVR32_SPI0_MR   = (0x01 | (1<<4)); // MSTR = 1, The rest have their default value... 
	// NOTE: SPI FREQ REDUCED, PLEASE CORRECT FOR PRODUCTION
	AVR32_SPI0_CSR0   = (0x02) | (0x04 << 8) | (0x08 << 4); // NPCHA = 1, CPOL = 0 (SPI MODE 0), SCBR = 100 (Clk/4=16MHz), BITS = 1000 (16 Bit mode)	
	...
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		...
		*/
		...
		AVR32_SPI1_MR   = (0x01 | (1<<4)); // MSTR = 1, The rest have their default value...
		// NOTE: SPI FREQ REDUCED, PLEASE CORRECT FOR PRODUCTION
		AVR32_SPI1_CSR0   = (0x02) | (0x04 << 8) | (0x08 << 4); // NPCHA = 1, CPOL = 0 (SPI MODE 0), SCBR = 100 (Clk/4=16MHz), BITS = 1000 (16 Bit mode)			
	#endif	
...
void __AVR32_ASIC_Activate_CS(char iBank)
{
	if (iBank == 1)
	{
		////JJ// AVR32_GPIO.port[0].ovrc   = AVR32_SPI0_PIN_NPCS;	
	}	
	#if defined(__PRODUCT_MODEL_SINGLE__) || defined(__PRODUCT_MODEL_MINIRIG__)
		if (iBank == 2)
		{
			////JJ// AVR32_GPIO.port[0].ovrc   = AVR32_SPI1_PIN_NPCS;	
		}		
	#endif
	/*
	NOP_OPERATION;
...
void __AVR32_ASIC_Deactivate_CS(char iBank)
{
	if (iBank == 1)
	{
		////JJ// AVR32_GPIO.port[0].ovrs   = AVR32_SPI0_PIN_NPCS;	
	}
	
	#if defined(__PRODUCT_MODEL_SINGLE__) || defined(__PRODUCT_MODEL_MINIRIG__)
		if (iBank == 2) 
		{
			////JJ// AVR32_GPIO.port[0].ovrs   = AVR32_SPI1_PIN_NPCS;
		}			
	#endif
	/*
	NOP_OPERATION;
...
unsigned short __AVR32_SPI1_ReadWord()
{
...
void MACRO__AVR32_SPI0_SendWord(u32 x) 
{ 
	AVR32_SPI0_TDR = (x & 0x0FFFF); 
	while ((AVR32_SPI0_SR & (1 << 9)) == 0); 
}
		
void MACRO__AVR32_SPI0_SendWord_Express(u32 x)
{
	AVR32_SPI0_TDR = (x & 0x0FFFF); 
...
u32 MACRO__AVR32_SPI0_ReadWord(u32 x)
{
...
void MACRO__AVR32_SPI1_SendWord(u32 x) 
		{ 
			AVR32_SPI1_TDR = (x & 0x0FFFF); 
			while ((AVR32_SPI1_SR & (1 << 9)) == 0); 
		}

void MACRO__AVR32_SPI1_SendWord_Express(u32 x) 
		{ 
			AVR32_SPI1_TDR = (x & 0x0FFFF); 
			while ((AVR32_SPI1_SR & (1 << 1)) == 0); 
		}

u32 MACRO__AVR32_SPI1_ReadWord(u32 x) 
		{ 
...
unsigned int __AVR32_SC_ReadData (char iChip, char iEngine, unsigned char iAdrs)
{
	...
	// Send it via SPI
		...
		// Generate the command
		iCommand = (ASIC_SPI_RW_COMMAND) |  // RW COMMAND (1 = Read)
					(((unsigned int)(iChip   & 0x0FF) << 12 ) &  0x7000) | // Chip address
					(((unsigned int)(iEngine & 0x0FF) << 8  ) &  0x0F00) |
					(((unsigned int)(iAdrs   & 0x0FF)       ) &  0x00FF); // Memory Address
...
void __AVR32_SC_WriteData_Express(char iChip, char iEngine, unsigned char iAdrs, unsigned int iData)
{
	unsigned int iCommand = 0;
		...
		{
			// RW COMMAND (0 = WRITE)
			iCommand = (((unsigned int)(iChip   & 0x0FF) << 12 ) &  0x7000) | // Chip address
					   (((unsigned int)(iEngine & 0x0FF) << 8  ) &  0x0F00) |
					   (((unsigned int)(iAdrs   & 0x0FF)       ) &  0x00FF); // Memory Address
...
void __AVR32_SC_WriteData(char iChip, char iEngine, unsigned char iAdrs, unsigned int iData)
{
	unsigned int iCommand = 0;
		...
		// Generate the command
		// RW COMMAND (0 = WRITE)
		iCommand = ((((u16)(iChip   & 0x0FF)) << 12 ) &  0x7000) | // Chip address
				   ((((u16)(iEngine & 0x0FF)) << 8  ) &  0x0F00) |
				   ((((u16)(iAdrs   & 0x0FF))       ) &  0x00FF); // Memory Address
...
{
	//JJ// AVR32_GPIO.port[0].ovrs  =  __AVR32_MAIN_LED_PIN;
}

void	__AVR32_MainLED_Reset()
{
	//JJ// AVR32_GPIO.port[0].ovrc  =  __AVR32_MAIN_LED_PIN;
}
...
void	__AVR32_LED_Set  (char iLed)
{
	#if defined(__PRODUCT_MODEL_LITTLE_SINGLE__) || defined(__PRODUCT_MODEL_JALAPENO__)
		if (iLed == 1){}
			//JJ// AVR32_GPIO.port[1].ovrs = __AVR32_ENGINE_LED1;
		else if (iLed == 2){}
			//JJ// AVR32_GPIO.port[1].ovrs = __AVR32_ENGINE_LED2;
		else if (iLed == 3){}
			//JJ// AVR32_GPIO.port[1].ovrs = __AVR32_ENGINE_LED3;
		else if (iLed == 4){}
			//JJ// AVR32_GPIO.port[1].ovrs = __AVR32_ENGINE_LED4;
		else if (iLed == 5){}
			//JJ// AVR32_GPIO.port[1].ovrs = __AVR32_ENGINE_LED5;
		else if (iLed == 6){}
			//JJ// AVR32_GPIO.port[1].ovrs = __AVR32_ENGINE_LED6;
		else if (iLed == 7){}
			//JJ// AVR32_GPIO.port[1].ovrs = __AVR32_ENGINE_LED7;
		else if (iLed == 8){}
			//JJ// AVR32_GPIO.port[1].ovrs = __AVR32_ENGINE_LED8;
...
void	__AVR32_LED_Reset(char iLed)
{
	#if defined(__PRODUCT_MODEL_LITTLE_SINGLE__) || defined(__PRODUCT_MODEL_JALAPENO__)	
		if (iLed == 1){}
			//JJ// AVR32_GPIO.port[1].ovrc = __AVR32_ENGINE_LED1;
		else if (iLed == 2){}
			//JJ// AVR32_GPIO.port[1].ovrc = __AVR32_ENGINE_LED2;
		else if (iLed == 3){}
			//JJ// AVR32_GPIO.port[1].ovrc = __AVR32_ENGINE_LED3;
		else if (iLed == 4){}
			//JJ// AVR32_GPIO.port[1].ovrc = __AVR32_ENGINE_LED4;
		else if (iLed == 5){}
			//JJ// AVR32_GPIO.port[1].ovrc = __AVR32_ENGINE_LED5;
		else if (iLed == 6){}
			//JJ// AVR32_GPIO.port[1].ovrc = __AVR32_ENGINE_LED6;
		else if (iLed == 7){}
			//JJ// AVR32_GPIO.port[1].ovrc = __AVR32_ENGINE_LED7;
		else if (iLed == 8){}
			//JJ// AVR32_GPIO.port[1].ovrc = __AVR32_ENGINE_LED8;
...
//{
	...
	// Clear the interrupt.
	//  unsigned int umx = AVR32_TC.channel[0].sr; // Read the SR flag to clear the interrupt
		
	//AVR32_RTC.val = 0;
	//  unsigned long uTop = AVR32_RTC.top;
	/*AVR32_RTC.top = -1;
...
int __AVR32_Timer_GetValue()
{
	...
	return 	0;   //AVR32_TC.channel[0].cv;
}
...
int	__AVR32_A2D_MUX_Convert()
{
	//#if defined(__PRODUCT_MODEL_LITTLE_SINGLE__) || defined(__PRODUCT_MODEL_JALAPENO__)
		// Not valid	
	//#else
	return	__AVR32_A2D_GetPWR_MAIN(); // A2D MUX OUTPUT is connected to PWR_MAIN for the LITTLE-SINGLE and JALAPENO 
	//#endif	
}
...
//
void __avr32_flash_writeRegister(uint16_t offset, int32_t value) {
	  int32_t *p = 0; //(int32_t *)(AVR32_FLASHC_ADDRESS + offset);
	*p = value;
}
...
void __avr32_flash_writeUserData(char* szStream)
{
	...
	// Copy data to buffer page
	  uint32_t *dst = 0;  //(uint32_t *)AVR32_FLASHC_USER_PAGE_ADDRESS;
	  //uint32_t bytes = 512;
...
void __avr32_flash_readUserData(char* szStream) 
{
	...
	// Copy data from buffer page
	  uint32_t *src = 0;  //(uint32_t *)AVR32_FLASHC_USER_PAGE_ADDRESS;
...
//
void __avr32_flash_waitForFlashReady() 
{
