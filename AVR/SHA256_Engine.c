...
int SHA256_Digest(char* szData, int iDataLen, int* retValsAtoH)
{
  ...
  unsigned int a, b, c, d, e, f, g, h;
  unsigned int v;
  int iTotalBitsThatShouldBe;
  int iTotalZerosToAdd;
  int iStartPositionFor64BitSize;
  int iFinalByteSize;
  unsigned int iBitSize ;
  unsigned int H_VALS[8];
  unsigned int umx;
  unsigned int byte_pos;
  unsigned int iGenerateWord;
  a = b = c = d = e = f = g = h = 0;
  ...
  for ( v = 0; v < 64; v++) sha_extention[v] = 0;
  ...
  iTotalBitsThatShouldBe = ((iDataLen * 8) + 1 + 64) ;
  iTotalZerosToAdd = 512 - (iTotalBitsThatShouldBe % 512);
  iStartPositionFor64BitSize = (iTotalZerosToAdd + 1) >> 3;
  iFinalByteSize = (iTotalBitsThatShouldBe + iTotalZerosToAdd) >> 3;
  
  // Put the 64Bit bit-size of the stream in the sha_extention
  iBitSize = (iDataLen << 3) & 0x0FFFFFFFF; // DataLen * 8
  ...
  for (umx = 0; umx < iFinalByteSize; umx += 64)
  {
	  for (byte_pos = umx; byte_pos < (umx + 64) && (umx < iDataLen); byte_pos += 4)	  
	  {
		  // Ok What we do here is we put the next 64 bytes in W0
		  // and then we expand the words
		  iGenerateWord = 0x0FFFFFFFF;
